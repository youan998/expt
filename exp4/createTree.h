...
#include <iostream>
#include <cstring>  // 补充：用于 strlen, strtok, strstr 等字符串处理函数
...
void preOrder(pTree &T,int v)//先序遍历访问下标为v的结点
{
	int w;
...
void preTraverse(pTree &T)//先序遍历访问树T
{
	int i;
	int visited[MAXLEN];
	for(i=0;i<T.n;i++)
...
void postOrder(pTree &T,int v)//后序遍历访问下标为v的结点

{
	...
	std::cout<<T.node[v].data<<"\t";   //访问根结点
}

void postTraverse(pTree &T)//后序遍历访问树T
{
	int i;
	int visited[MAXLEN];
	for(i=0;i<T.n;i++)
...
//*******************************************************************//
int CreateTreeFromFile(char fileName[], pTree &T)//从文本文件创建双亲表示的树（森林）
{
